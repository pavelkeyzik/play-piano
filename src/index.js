...
const notes = {
  ...
  G: [24.5, 49.0, 98.0, 196.0, 392, 783.99, 1567.98, 3135.96, 6271.93],
  "G#": [
    25.96, 51.91, 103.83, 207.65, 415.3, 830.61, 1661.22, 3322.44, 6644.88,
  ],
  A: [27.5, 55.0, 110.0, 220.0, 440, 880.0, 1760.0, 3520.0, 7040.0],
  "A#": [
...
async function playChord(chord, octave, scale, time = 3000) {
  ...
  if (scale === "major") {
    ...
    activeNotes.push({ note: chord, octave: octave });
    renderNotes();
  ...
  activeNotes.push({ note: note1, octave: note1Octave });
  activeNotes.push({ note: note2, octave: note2Octave });
  activeNotes.push({ note: note3, octave: note3Octave });
...
function renderNotes() {
  ...
  for (let currentOctave = 0; currentOctave < 6; currentOctave++) {
    for (let i = 0; i < 12; i++) {
      ...
      if (
        activeNotes.find(
          (item) => item.note === noteName && item.octave === currentOctave
        )
      ) {
